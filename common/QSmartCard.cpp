...
void WinCard::run()
{
    ...
    while (isRunning)
    {
        QCNG::Certs tmp = win.certs();
        for(QCNG::Certs::iterator i = tmp.begin(); i != tmp.end();)
        {
            if(!i.key().enhancedKeyUsage().contains(SslCertificate::ClientAuth))
                i = tmp.erase(i);
            else
                ++i;
        }
...
{
    for(QCNG::Certs::const_iterator i = cache.cbegin(); i != cache.cend(); ++i)
    {
        if(i.value() == card)
        {
            TokenData token;
            token.setCard(i.value());
            token.setCert(i.key());
            d->selected = token;
            win.selectCert(i.key());
        }
    }
}
