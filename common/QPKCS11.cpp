...
QPKCS11::PinStatus QPKCS11::login(const TokenData &_t, PinDialogFactory &pdf)
{
	logout();
	...
	{
        QScopedPointer<PinDialogInterface> p(pdf.createPinDialog(pin2 ? PinDialogInterface::Pin2PinpadType : PinDialogInterface::Pin1PinpadType, t.cert()));
        p->doStartTimer();
        p->execDialog();
    ...
    {
        QScopedPointer<PinDialogInterface> p(pdf.createPinDialog(pin2 ? PinDialogInterface::Pin2Type : PinDialogInterface::Pin1Type, t.cert()));
#if 0
		p.setMinPinLen(token.ulMinPinLen);
#endif
        if(!p->execDialog())
			return PinCanceled;
        QByteArray pin = p->getPin();
...
bool QPKCS11Stack::load()
{
#ifdef Q_OS_MAC
    d->drivers.insert("/Library/OpenSC/lib/opensc-pkcs11.so", QByteArray());
    ...
    d->drivers.insert("opensc-pkcs11.dll", QByteArray());
...
#else
    d->drivers.insert("opensc-pkcs11.so", QByteArray());
...
QPKCS11::PinStatus QPKCS11Stack::login(const TokenData &t, PinDialogFactory &pdf)
{
    return d->active ? d->active->login(t, pdf) : QPKCS11::UnknownError;
}
